...
class Solution:
    def minDepth(self, root: Optional[TreeNode]) -> int:
        ...
        # layer_1 代表此次遍历； layer_2 代表下次遍历。这种写法比用queue更方便
        layer_1, layer_2 = [root], []
        
        while layer_1:
            min_depth += 1
            for node in layer_1:
                if node.left is None and node.right is None: # 此时发现叶节点
                    return min_depth
                
                # 不是叶节点则增加其子节点到下一次遍历
                if node.left:
                    layer_2.append(node.left)
                if node.right:
                    layer_2.append(node.right)
